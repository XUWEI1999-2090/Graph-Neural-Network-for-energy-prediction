...
class GraphNeuralNetwork(nn.Module):
    def __init__(self,
                 ...
                 with_batch_norm=False,  # 是否使用batchNorm
        ...
        self.W_output = nn.ModuleList([nn.Linear(dim+1, dim+1)
                                       for _ in range(output_layer)])
        '''
        上面两个可以看成是全连接层的数组，之后会用到，同时激活函数也在后面设置
        '''
        self.W_property = nn.Linear(dim+1, 1)  # 回归问题，最终输出为标量
        self.device = device
    ...
    def forward(self, fingerprints, adjacencies, metal_feature, embedding=False):
        ...
        for i in range(len(fingerprints)):
            metal_vector = np.append(metal_vector, np.repeat(metal_feature[i], len(fingerprints[i])))
        metal_vector = torch.tensor(np.transpose([metal_vector]), dtype=torch.float32)
        ...
        self.f_fingerprint_vectors = torch.cat((self.f_fingerprint_vectors, metal_vector), dim=1)
        ...
        for j in range(self.output_layer):
            if self.bn:
                self.f_molecular_vectors = nn.BatchNorm1d(self.dim+1)(
    ...
    def __call__(self,
        ...
        metal_feature = [i.metal_label for i in data_batch]
        ...
        if output_embedding:
            return self.forward(fingerprints, adjacencies, metal_feature, embedding=True)

        predicted_properties = self.forward(fingerprints, adjacencies, metal_feature)
